...
- MVP 阶段主要操作 `papers` 主表，并新增 `paper_upload_parse_progress` 表记录每次上传请求的 `trace_id`、`paper_md5`、文件名和提交时间；Zotero 写入成功后写入论文元数据，并将状态置为解析中；MinerU 解析、MinIO 保存、向量化和 Elasticsearch 写入完成后，只回写状态字段。
...
flowchart LR
    ...
    Z --> X[文献导入服务]
    B --> X
    X --> P0[(MySQL<br/>paper_upload_parse_progress<br/>上传解析进度)]
...
| 元数据存储 | MySQL 8 | 保存论文元数据、上传解析进度、章节、chunk、引用、任务状态和检索日志 |
...
- DocumentImportService：统一处理 Zotero 同步和本地上传两类入口，负责接收前端 `traceId`、记录上传解析进度、去重、写入 `papers` 主表并创建解析任务。
...
MinIO 建议只保存文件对象，不承担业务查询职责。MVP 阶段 MySQL 主要操作 `papers` 主表，并用 `paper_upload_parse_progress` 记录每次上传请求的 `trace_id`、`paper_md5`、`filename` 和 `submission_time`，便于前端或日志系统按一次上传请求追踪进度。Zotero 写入成功后，直接写入论文基础字段、Zotero 映射字段，并把状态字段置为 `1` 表示解析中；MinerU 解析、MinIO 保存、向量化、写入 Elasticsearch 完成或失败后，只修改状态字段。`paper_attachments`、`paper_artifacts`、`sections`、`chunks` 等表留到后续迭代再拆分。
...
1. 用户通过前端/API 上传 PDF，前端在表单字段最前面提交 `traceId`。
2. 后端计算 `paper_md5`，用于去重和文件标识。
3. DocumentImportService 写入 `paper_upload_parse_progress`，记录 `trace_id`、`paper_md5`、`filename` 和 `submission_time`。
4. ZoteroSyncService 先创建或更新 Zotero item，并上传 PDF attachment；如果 Zotero 写入失败，直接抛出错误，不写 `papers` 主表。
5. PaperService 写入 `papers` 主表：基础元数据、Zotero item / attachment 信息、`paper_md5` 等字段直接入库，并将状态字段置为 `1`，表示解析中。
6. IngestionService 使用上传文件或 Zotero attachment 可读文件调用 MinerU API 解析。
7. ObjectStorageService 将原始 PDF 和 MinerU 产物保存到 MinIO；暂不拆分写入 `paper_artifacts`。
8. 如果 MinIO 对象地址需要落主表，必须使用写入 `papers` 前已预生成的 bucket / object key。
9. 根据 MinerU 结果生成可向量化文本。
10. 使用 bge-m3 或当前选定 embedding 模型生成向量。
11. 将 chunk 文本、向量和必要 metadata 写入 Elasticsearch。
12. PaperService 回写 `papers` 主表状态字段：成功、失败或待重试；除状态字段外，其它字段本轮不再修改。

后续完整流程可以再恢复章节表、chunk 表、artifact 表、Crossref/OpenAlex 元数据补全和独立向量库；当前阶段先把“上传携带 traceId -> 上传解析进度表 -> Zotero -> 主表状态=解析中 -> MinerU -> MinIO 保存原始文件和 MinerU 产物 -> 向量化 -> Elasticsearch -> 主表状态”跑通。
...
MVP 当前约束：先落 `papers` 主表和 `paper_upload_parse_progress` 上传解析进度表。文件导入时先按前端提交的 `traceId` 记录一次上传请求，再写入主表可获得的字段，并将 `parse_status` 置为 `1` 表示解析中；MinerU、向量化和 Elasticsearch 写入结束后，只更新 `parse_status`。下面的其它表是后续规范化拆表方向，不作为当前第一版强制实现。

核心表：

- papers：论文基本信息和 Zotero item 映射。
- paper_upload_parse_progress：上传解析进度入口表，按 `trace_id` 记录每次上传请求的 `paper_md5`、`filename` 和 `submission_time`。
...
paper_upload_parse_progress 第一版字段：
...
Zotero 是本项目的文献管理端，MinIO 保存 ScholarEase 解析后的原始文件副本和 MinerU 产物。当前 MVP 对本地上传采用固定顺序：前端先提交 `traceId`，后端写入 `paper_upload_parse_progress` 记录上传解析进度入口；再写 Zotero，Zotero 失败则直接抛出错误且不写 `papers` 主表；Zotero 成功后写 MySQL `papers` 主表并置为解析中；MinerU 解析后再写 MinIO；向量化和 Elasticsearch 写入完成后，只回写 `papers` 的状态字段。
...
- MVP 阶段 MySQL 维护 `papers` 主表和 `paper_upload_parse_progress` 上传解析进度表；Zotero item、attachment 和 MinIO 对象映射暂时仍放在主表中。
...
Zotero item
  -> ZoteroSyncService
  -> insert / update MySQL paper_upload_parse_progress when a trace_id exists
...
Local PDF upload
  ...
  -> receive traceId / fileName / paperMd5 / fileSizeBytes / submissionTime
  -> insert MySQL paper_upload_parse_progress
...
- MySQL 第一版操作 `papers` 主表和 `paper_upload_parse_progress` 上传解析进度表：导入时先记录 traceId 维度进度入口，再将主表状态置为解析中，流程结束后只回写主表状态字段。
- MinerU API 解析 PDF。
- MinerU 解析后，将原始 PDF 和 MinerU 产物写入 MinIO。
...
ScholarEase 采用 Vue 3 前端、Java 17 + Spring Boot 3 后端、MySQL 保存论文元数据与上传解析进度、MinIO 保存原始 PDF 与 MinerU 产物、Redis 对话历史、Zotero 双向同步文献、MinerU API 解析论文结构、Crossref API + OpenAlex API 补全文献元数据、bge-m3 生成向量、Elasticsearch 同时保存 chunk 文本和 dense_vector 向量并承担 BM25 + kNN 检索、bge-reranker-v2-m3 做精排，并尽可能用 LangChain / LangChain4j 编排 RAG 流程。
