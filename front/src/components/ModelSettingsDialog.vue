 ...
 * 模型设置对话框组件。
 *
 * 提供一个可视化的配置面板，允许用户调整当前使用的 DeepSeek 对话模型参数，
 * 包括 API 地址、密钥、模型名称、温度、Top-P、最大 Token 数和超时时间。
 *
...
<template>
  ...
  <el-dialog v-model="visible" title="模型设置" width="760px" align-center>
    <el-form class="model-form" label-position="top">
      ...
      <div class="form-grid">
        <!-- API 密钥输入，密码模式显示；留空时后端使用环境变量中的默认密钥 -->
        ...
        <el-form-item label="modelName">
          ...
          >
            <el-option
              v-for="model in modelSettings.modelNames"
              :key="model"
              :label="model"
              :value="model"
            />
    ...
    <div class="provider-hint">
      <strong>{{ modelSettings.providerLabel }}</strong>
      <span>{{ modelSettings.description }}</span>
    </div>
...
/* 双列网格布局，用于 apiKey/modelName 等成组表单项 */
.form-grid {
  display: grid;
